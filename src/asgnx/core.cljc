...
;; 
;; helper function for the format-hour function
;;  turns 0-23 hour time to 12 hour representation
;; 
;;	0 or 12 -> 12
;;  else h -> h % 12
;;
(defn format-hour-1 [h]
	(if (or (= h 0) (= h 12))
		12
		(mod h 12)
	)
...
(defn office-hours [{:keys [args cmd]}]
	(let [hours (get instructor-hours (first args))]
		(if (nil? hours)
